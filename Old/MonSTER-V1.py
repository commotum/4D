...
def clifford_rope(x, coords, times, base: float = 10000.):
    ...
    *pref, d = x.shape
    ...
    # Reshape coords to match the block structure
    coords_expanded = jnp.broadcast_to(coords, (*pref, B, 3))
    theta_vec = coords_expanded * invf[..., None]
    ...
    # --- Temporal Boost Part ---
    # Reshape times to match the block structure
    ...
    M_boost = M_boost.at[..., 0, 1:].set(-axis_u * sh[..., None]) # Sign fixed for (+,---)
    M_boost = M_boost.at[..., 1:, 0].set(-axis_u * sh[..., None]) # Sign fixed for (+,---)
